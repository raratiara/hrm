...
class Unbind_devices_menu extends MY_Controller
{
	/* Costs */
 	const  LABELMODULE				= "unbind_devices_menu"; // identify menu
 	const  LABELMASTER				= "Unbind Devices";
 	const  LABELFOLDER				= "general_system"; // module folder
 	const  LABELPATH				= "unbind_devices_menu"; // controller file (lowercase)
 	const  LABELNAVSEG1				= "general_system"; // adjusted 1st sub parent segment
	...
	public function form_field_asset()
	{
		...
		$oKaryawan 				= $this->db->select('a.id as id, a.full_name,b.name as description')->join(_PREFIX_TABLE."master_job_title".' b','b.id=a.job_title_id','left')->order_by('a.full_name', 'ASC')->get(_PREFIX_TABLE."employees a")->result();
		$field['selkaryawan'] 	= $this->self_model->return_build_select2me($oKaryawan,'','','','id_karyawan','id_karyawan','','','id','full_name','-','description');
	...
	//============================== Additional Method ==============================//
 	...
 	public function unBind(){
		...
		if($id != ''){
			...
			$rs = $this->db->update('user_devices', $data, "id = '".$id."'");
