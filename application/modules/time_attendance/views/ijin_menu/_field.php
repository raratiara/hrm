<div class="row">
	
	<div class="col-md-6 col-sm-12">
		<div class="form-group">
			<label class="col-md-4 control-label no-padding-right">Employee Name</label>
			<div class="col-md-8">
				<?=$selemployee;?>
			</div>
		</div>
		
		<div class="form-group">
			<label class="col-md-4 control-label no-padding-right">Date Leave Start</label>
			<div class="col-md-8">
				<?=$txtdatestart;?>
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-4 control-label no-padding-right">Reason</label>
			<div class="col-md-8">
				<?=$txtreason;?>
			</div>
		</div>
		<span style="color:red" class="sisa_cuti"></span>
		
	</div>
	<div class="col-md-6 col-sm-12">
		<div class="form-group">
			<label class="col-md-4 control-label no-padding-right">Leave Type</label>
			<div class="col-md-8">
				<?=$selleavetype;?>
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-4 control-label no-padding-right">Date Leave End </label>
			<div class="col-md-8">
				<?=$txtdateend;?>
			</div>
		</div>
		
		<div class="form-group">
			<label class="col-md-4 control-label no-padding-right">Attachment</label>
			<div class="col-md-4">
				<?=$attachment;?>
				<input type="hidden" id="hdnattachment" name="hdnattachment"/>
			</div>
			<div class="col-md-4">
				<span class="file_attachment"></span>
			</div>
		</div>

		
		
	</div>
</div>



								