...
class Group_shift_schedule_os_menu_model extends MY_Model
{
	/* Module */
 	protected $folder_name				= "time_attendance/group_shift_schedule_os_menu";
 	protected $table_name 				= _PREFIX_TABLE."group_shift_schedule_os";
		...
		if($post['bulan'] != '' && $post['tahun'] != ''){
			...
			if($period != ''){
				if(!empty($grouped)){
					...
					{
						$exist_data = $this->db->query("select * from group_shift_schedule_os where periode = '".$period."' and employee_id = '".$groupedRow['id']."'")->result(); 
	...
	public function add_data_old($post) { 
		...
		if($period != '' && $group != ''){
			$exist_data = $this->db->query("select * from group_shift_schedule_os where periode = '".$period."' and master_group_shift_id = '".$group."'")->result(); 
					...
					if($item_num2>0){
						for($j=$item_len_min2;$j<=$item_len2;$j++) 
						{
							...
							if(isset($post['employee'][$j])){
								...
								$this->db->insert('shift_schedule_os', $itemData2);
							}
						}
					}
		...
		if($period != ''){
			...
			foreach($grouped as $groupedRow)
			{
				...
				if(empty($exist_data)){
					...
					$del = $this->db->delete('group_shift_schedule_os',"id = '".$exist_data[0]->id."'");
					...
					for($j=$item_len_min2;$j<=$item_len2;$j++) 
					{
						...
						if(!empty($hdnid)){ //update
							if(isset($post['employee'][$j])){
								...
								$this->db->update('shift_schedule_os', $itemData2, "id = '".$hdnid."'");
							}
						}else{ //insert
							if(isset($post['employee'][$j])){
								$itemData2 = [
	...
	public function getRowData_old($id) { 
		/*$mTable = '(select a.*, b.name as group_name from group_shift_schedule_os a 
						left join master_group_shift b on b.id = a.master_group_shift_id
		...
		$rs = $this->db->query("select a.*, b.name as group_name from group_shift_schedule_os a 
	...
	{
		$sql = 'select a.*, b.name as group_name from group_shift_schedule_os a 
				left join master_group_shift b on b.id = a.master_group_shift_id
				order by a.id asc
		';
	...
	{ 
		if($id > 0){ 
			...
			$data = '';
			$no = $row+1;
			$msEmp = $this->db->query("select * from employees where emp_source = 'outsource' and status_id = 1 and shift_type = 'Shift'")->result(); 
	...
	public function getEmplistRows($id,$view,$print=FALSE){ 
		...
		$rs = $this->db->query("select a.*, b.full_name from shift_schedule_os a 
								left join employees b on b.id = a.employee_id
								where a.group_shift_schedule_id = '".$id."' and b.emp_source = 'outsource' ")->result(); 
