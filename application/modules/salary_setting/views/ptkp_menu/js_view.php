...
jQuery(function($) {
	...
	validator = $("#frmInputData").validate({
		...
		rules: { status_code: { required: true }, amount: { required: true } },
		messages: {},
		...
		success: function(data) {
			if(data != false){
				...
				if(save_method == 'detail'){
					...
					$('span.amount').html(parseFloat(data.amount || 0).toLocaleString('id-ID'));
					$('span.effective_year').html(data.effective_year);
					$('span.marital_status_name').html(data.marital_status_name);
					$('#modal-view-data').modal('show');
				}
			} else {
				var dialog = bootbox.dialog({ message: '<div class="text-center" style="padding-top:20px;padding-bottom:10px;"><i class="fa fa-exclamation-circle fa-5x" style="color:red"></i></div><center><p>Gagal peroleh data.</p><br/><button class="btn blue" data-dismiss="modal">OK</button></center>' });
			}
		},
		error: function (jqXHR) {
			bootbox.dialog({ title: 'Error ' + jqXHR.status + ' - ' + jqXHR.statusText, message: jqXHR.responseText, buttons: { confirm: { label: 'Ok', className: 'btn blue' } } });
		}
	});
}
<?php } ?>
