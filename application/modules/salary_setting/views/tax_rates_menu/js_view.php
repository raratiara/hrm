...
jQuery(function($) {
	...
	validator = $("#frmInputData").validate({
		...
		rules: { category: { required: true }, min_bruto: { required: true }, max_bruto: { required: true }, rate: { required: true } },
		messages: {},
		...
		success: function(data) {
			if(data != false){
				if(save_method == 'update'){
					...
					$('[name="min_bruto"]').val(data.min_bruto);
					$('[name="max_bruto"]').val(data.max_bruto);
				...
				if(save_method == 'detail'){
					...
					$('span.min_bruto').html(parseFloat(data.min_bruto || 0).toLocaleString('id-ID'));
					$('span.max_bruto').html(parseFloat(data.max_bruto || 0).toLocaleString('id-ID'));
