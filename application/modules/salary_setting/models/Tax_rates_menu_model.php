...
class Tax_rates_menu_model extends MY_Model
{
	protected $folder_name	= "salary_setting/tax_rates_menu";
	protected $table_name 	= "tax_ter";
	...
	{
		$aColumns = [NULL, NULL, 'dt.id', 'dt.category', 'dt.min_bruto', 'dt.max_bruto', 'dt.rate', 'dt.effective_year'];

		$sIndexColumn = $this->primary_key;
		$sTable = '(SELECT * FROM tax_ter ORDER BY id ASC)dt';
		...
		foreach($rResult as $row) {
			...
			array_push($output["aaData"], array(
				...
				$row->id, $row->category, number_format($row->min_bruto, 0, ',', '.'), number_format($row->max_bruto, 0, ',', '.'), $row->rate, $row->effective_year
			));
		}
	...
	public function add_data($post) {
		if(!empty($post['category'])){
			$data = [
				...
				'min_bruto' => str_replace(['.', ','], ['', '.'], trim($post['min_bruto'] ?? '0')),
				'max_bruto' => str_replace(['.', ','], ['', '.'], trim($post['max_bruto'] ?? '0')),
		...
		} else return ["status" => false, "msg" => "Category harus diisi"];
	}
	...
	public function getRowData($id) { return $this->db->where([$this->primary_key => $id])->get('(SELECT * FROM tax_ter)dt')->row(); }

	public function import_data($list_data) {
		$error = '';
		foreach ($list_data as $k => $v) {
			$data = ['category' => $v["B"], 'min_bruto' => $v["C"], 'max_bruto' => $v["D"], 'rate' => $v["E"], 'effective_year' => $v["F"], 'created_at' => date("Y-m-d H:i:s")];
	...
	public function eksport_data() {
		return $this->db->query("SELECT * FROM tax_ter ORDER BY id ASC")->result_array();
	}
}
