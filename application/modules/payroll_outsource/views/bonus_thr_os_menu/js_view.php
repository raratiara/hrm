		...
		success: function(data)
		{
			if(data != false){
				if(save_method == 'update'){
					...
					$('input[name="component_type"][value="'+data.component_type+'"]').prop('checked', true);
				...
				if(save_method == 'detail'){
					...
					$('span.component_type').html(data.component_type);
...
function loadBonusThrRows(id, project, view)
{
	...
	$.ajax({
		...
		data: { id: id, project: project, view: view },
...
$('#modal-form-data').on('shown.bs.modal', function () {
	initSelect2(this);
	if(save_method == 'add'){
		$('table.bonusthr-list tbody').html('<tr><td colspan="5" class="center">Pilih project terlebih dahulu</td></tr>');
		setBonusThrTotal();
	}
});
...
function setBonusThrTotal() {
	var totalBonus = 0;
	var totalThr = 0;

	$('input[name^="bonus_amount"]').each(function() {
		totalBonus += toNumber($(this).val());
	});

	$('input[name^="thr_amount"]').each(function() {
		totalThr += toNumber($(this).val());
	});

	$('#total_bonus_text').text(totalBonus.toLocaleString('id-ID'));
	$('#total_thr_text').text(totalThr.toLocaleString('id-ID'));
}
