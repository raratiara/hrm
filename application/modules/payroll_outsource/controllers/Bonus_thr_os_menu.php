...
class Bonus_thr_os_menu extends MY_Controller
{
	const LABELMODULE = "bonus_thr_os_menu";
	const LABELMASTER = "Menu Bonus & THR Outsource";
	const LABELFOLDER = "payroll_outsource";
	const LABELPATH = "bonus_thr_os_menu";
	...
	public $icon = 'fa-gift';
	public $tabel_header = ["ID", "Project", "Periode", "Jenis", "Total Nominal"];

	public $colnames = ["ID", "Project", "Bulan", "Tahun", "Jenis", "Total Nominal"];
	public $colfields = ["id", "project_name", "month_name", "periode_tahun", "component_type", "total_nominal"];
	...
	public function form_field_asset()
	{
		...
		$field['selcomponenttype'] = $this->self_model->return_build_radio('Bonus', [['Bonus','Bonus'], ['THR','THR']], 'component_type', '', 'inline');
	...
	public function genbonusrowthr()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1") {
			...
			$type = isset($post['component_type']) ? trim($post['component_type']) : 'Bonus';
			$view = (isset($post['view']) && $post['view'] == TRUE) ? TRUE : FALSE;

			echo json_encode($this->self_model->getNewBonusThrRows($id, $project, $view, $type));
		} else {
