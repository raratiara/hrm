...
class Data_karyawan_menu_model extends MY_Model
{
	...
	private function seed_employee_benefit_deduction($employee_id)
	{
		if(empty($employee_id) || !$this->db->table_exists('employee_benefit_deduction')){
			return false;
		}
		...
		$components = $this->db->query("select id from salary_components
					where is_active = 1
		...
		if($post['full_name'] == '' || $post['status'] == '' || $post['no_ktp'] == '' || $post['marital_status'] == '' || $post['no_bpjs'] == '' || $post['no_bpjs_ketenagakerjaan'] == '' || $post['work_loc'] == '' || $post['shift_type'] == '' || $post['date_of_hire'] == ''){
			...
			if(!empty($dataEmployee)){
				...
				if($rs){
					...
					//add default benefit/deduction component rows
					$this->seed_employee_benefit_deduction($lastId);
					//end default benefit/deduction component rows
	...
	public function getRowData($id) { 
		$mTable = '(SELECT 
					    ...
					    end) as is_tracking_name, u.username, s.name as work_location_name
					FROM
					    employees a
					        LEFT JOIN
					    ...
					    left join master_work_location s on s.id = a.work_location
	...
	public function import_data($list_data){  
		...
		for ($i = 2; $i < count($list_data); $i++) {
            ...
            /// UPDATE DATA
         	if($row[62] != '' && $list_data[0][62] == 'Employee Code') 
         	{ 
            	...
            	$employee_id = $getID[0]->id;
         		...
         		if($employee_id != ''){ 
					...
					$upload_dir = './uploads/employee/'.$row[62].'/'; // nama folder
					// Cek apakah folder sudah ada
					if (!is_dir($upload_dir)) {
					    // Jika belum ada, buat folder
					    mkdir($upload_dir, 0755, true); // 0755 = permission, true = recursive
