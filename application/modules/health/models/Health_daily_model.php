...
class Health_daily_model extends MY_Model
{
	/* Module */
 	protected $folder_name				= "health/Health_daily";
 	protected $table_name 				= _PREFIX_TABLE."health_daily";
	...
	{ 
		$aColumns = [
			...
			'dt.created_at',
			'dt.updated_at',
			'dt.last_sync_runs_id'
		];
		
		

		$sIndexColumn = $this->primary_key;
		$sTable = '(select a.*, b.full_name from health_daily a left join employees b on b.id = a.employee_id
				)dt';
	...
	// 	$mTable = '(select a.*, b.full_name from health_sync_runs a left join employees b on b.id = a.employee_id
	// 		)dt';
	...
	{ 
		...
		$sql = "select a.*, b.full_name from health_daily a left join employees b on b.id = a.employee_id
				ORDER BY a.id desc
		";
