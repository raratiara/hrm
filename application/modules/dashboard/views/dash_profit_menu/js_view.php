...
<script type="text/javascript">
	...
	function dataTotal(flcust, is_all_project, project) {


		$.ajax({
			type: "POST",
			url: module_path + '/get_data_total',
			data: { flcust: flcust, is_all_project: is_all_project, project: project},
			cache: false,
			dataType: "JSON",
			success: function (data) {
				if (data != false) {

					$('span#ttl_emp').html(data.ttl_emp);
					$('span#ttl_biaya').html(data.ttl_biaya);
					$('span#ttl_tagihan').html(data.ttl_tagihan);
	...
	function monthlyProfitAmount(flcust, is_all_project, project) {



		$.ajax({
			type: "POST",
			url: module_path + '/get_data_monthlyProfitAmount',
			data: { flcust: flcust, is_all_project: is_all_project, project: project},
			cache: false,
			dataType: "JSON",
			success: function (data) {
				if (data != false) {
					...
					const ctx = document.getElementById('monthly_profit_amount').getContext('2d');

					var chartExist = Chart.getChart("monthly_profit_amount"); // <canvas> id
					if (chartExist != undefined)
						chartExist.destroy();
	...
	document.querySelectorAll('input[name="is_all_project"]').forEach(function(radio) {
	  radio.addEventListener('click', function() {
	  		...
	  		var flcust = $("#flcust option:selected").val();

		  	if(this.value == 'Sebagian'){
	...
	function generateProfit(){
		var flcust = $("#flcust option:selected").val();
		var is_all_project = $('input[name="is_all_project"]:checked').val();


		if(flcust != '' && is_all_project != ''){

			/*document.getElementById("summary-container-id").style.display = "block";
			document.getElementById("employee-container-id").style.display = "block";*/


			var project = '';
			if(is_all_project == 'Sebagian'){
				project = $("#projectIds").val(); 
			}
 	
			dataTotal(flcust, is_all_project, project);
			monthlyProfitAmount(flcust, is_all_project, project);
			...
			document.getElementById("employee-container-id").style.display = "none";*/
		}
		
	}


	$('#flcust').on('change', function () {
