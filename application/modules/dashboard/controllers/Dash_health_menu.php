...
class Dash_health_menu extends MY_Controller
{
 	...
 	public function get_data_vitalSigns()
	{
 	...
 	public function get_data_heart_rate(){
 		$post = $this->input->post(null, true);
 		$fldiv 	= $post['fldiv'];
    	...
    	$rs = $this->db->query("select tanggal, ts_utc, bpm
								FROM (
								    SELECT 
								        DATE_FORMAT(ts_utc, '%Y-%m-%d') AS tanggal,
								        ts_utc,
								        bpm,
								        ROW_NUMBER() OVER (PARTITION BY DATE(ts_utc) ORDER BY ts_utc, id DESC) AS rn
								    FROM health_raw_hr
								    WHERE employee_id = 2
								) t
								WHERE rn = 1 and tanggal is not null
