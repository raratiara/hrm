...
class Dashboard_menu extends MY_Controller
{
	...
	//============================== Additional Method ==============================//
 	...
 	public function get_data_total(){
		...
		$whr_emp=""; $whr_att=""; $whr_latelogin=""; $whr_earlylogin=""; 
		...
		$ttl_attendance = $this->db->query("select count(distinct(a.date_attendance)) as ttl_absences_days from time_attendances a left join employees b on b.id = a.employee_id where b.emp_source = 'internal' ".$whr_att.$whr2_att." ")->result(); 
		$ttl_latelogin = $this->db->query("select count(a.id) as ttl from time_attendances a left join employees b on b.id = a.employee_id where a.is_late = 'Y' and b.emp_source = 'internal' ".$whr_latelogin.$whr2_latelogin." ")->result(); 
		$ttl_earlylogin = $this->db->query("select count(a.id) as ttl from time_attendances a left join employees b on b.id = a.employee_id where b.emp_source = 'internal' and a.date_attendance_in is not null and (a.is_late != 'Y' or a.is_late = '' or a.is_late is null) ".$whr_earlylogin.$whr2_earlylogin." ")->result(); 
 	...
 	public function get_data_workLoc(){
		...
		$data_att = $this->db->query("select a.* from time_attendances a left join employees b on b.id = a.employee_id where b.emp_source = 'internal' ".$whr.$whr_emp."")->result(); 
 	...
 	public function get_data_monthlyAttSumm(){
    	...
    	$rs = $this->db->query("select
				...
				FROM
				    time_attendances a left join employees b on b.id = a.employee_id where b.emp_source = 'internal' and a.date_attendance <= '".$dateNow."'
				".$where_date.$where_emp."
				GROUP BY
 	...
 	public function get_data_dailyAttSumm(){
    	...
    	$rs = $this->db->query("select 
					    ...
					    COUNT(*) AS total_karyawan
					FROM time_attendances a left join employees b on b.id = a.employee_id 
					where b.emp_source = 'internal' and a.date_attendance <= '".$dateNow."' 
					".$where_date.$where_emp."
 	...
 	public function get_data_attStatistic(){
    	...
    	$rs = $this->db->query("select
						...
						(
						    ...
						    WHERE b.emp_source = 'internal' and a.date_attendance IS NOT NULL
						    ".$where_date." ".$where_emp."
 	...
 	public function get_data_attPercentage(){
    	...
    	$rs = $this->db->query("select
				...
				FROM time_attendances a left join employees b on b.id = a.employee_id where b.emp_source = 'internal' ".$where_date.$where_emp."
				 ")->result(); 
