...
class Hitung_gaji_int_menu extends MY_Controller
{
	/* Module */
 	const  LABELMODULE				= "hitung_gaji_int_menu"; // identify menu
 	const  LABELMASTER				= "Menu Hitung Gaji";
 	const  LABELFOLDER				= "payroll_internal"; // module folder
 	const  LABELPATH				= "hitung_gaji_int_menu"; // controller file (lowercase)
	...
	public $tabel_header 			= ["ID", "Bulan Penggajian","Tahun Penggajian","Status"];

	
	/* Export */
	public $colnames 				= ["ID", "Bulan Penggajian","Tahun Penggajian","Status"];
	public $colfields 				= ["id","month_name","tahun_penggajian","status"];
	...
	{ 
		...
		{ 
			...
			{  
				$row = trim($post['count']); 
				echo $this->self_model->getNewGajiRow($row);
			} else if(isset($post['id'])) { 
				$row = 0;
				$id = trim($post['id']);
				$view = (isset($post['view']) && $post['view'] == TRUE)? TRUE:FALSE;
				echo json_encode($this->self_model->getNewGajiRow($row,$id,$bln,$thn,$view));
			}
		}
		else
		{ 
	...
	public function getPayrollReport_pdf()
	{
        ...
        $sql = "
			...
			WHERE b.emp_source = 'internal' and IFNULL(b.is_special_payroll,0) != 1 and a.id = ".$_GET['payroll_id']."
        ...
        header("Content-Disposition: attachment; filename=GAJI - Semua Karyawan.pdf");
	...
	public function getPayrollReport_perEmployee_pdf()
	{
		...
		isset($_GET['id']) && $_GET['id'] != '' && $_GET['id'] != 0){
		    ...
		    $sql = "
				...
				left join master_month c on c.id = a.bulan_penggajian
				left join master_job_title e on e.id = b.job_title_id
				where b.emp_source = 'internal' and IFNULL(b.is_special_payroll,0) != 1 and a.id = ".$_GET['id']." and bb.employee_id = ".$_GET['emp_id']."  
		    ";
	...
	public function getAbsenceReportGaji_pdf()
	{
	    ...
	    // ================= PDF DATA =================
	    $pdfData = [
	    	'periode_penggajian' => $data[0]->periode_penggajian,
	        'datax' => [[
	            'summary'      => $valSummary
	        ]]
	    ];
	...
	public function getAbsenceReportGaji_pdf_zip()
	{
	    ...
	    if (!empty($_GET['payroll_id'])) {
	        ...
	        $sql = "
				...
				left join employees b on b.id = bb.employee_id
				where b.emp_source = 'internal' and IFNULL(b.is_special_payroll,0) != 1 and a.id = ".$_GET['payroll_id']." and b.project_id = a.project_id
	...
	public function getRekapGaji_pdf()
	{
	    if (empty($_GET['payroll_id'])) {
	        echo "<script>
		        alert('Data tidak ditemukan');
	    ...
	    $sql = "
			...
			where c.emp_source = 'internal' and IFNULL(c.is_special_payroll,0) != 1 and a.id = ".$_GET['payroll_id']." 
