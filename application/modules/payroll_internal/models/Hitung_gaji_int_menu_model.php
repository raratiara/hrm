...
class Hitung_gaji_int_menu_model extends MY_Model
{
	...
	protected $approval_matrix_type_id 	= 18;
	protected $payment_history_table 	= _PREFIX_TABLE."payroll_paid_history_internal";
	...
	{
		if(!$this->db->field_exists('pph21_adjustment', 'payroll_slip_detail_internal')){
			$this->db->query("ALTER TABLE `payroll_slip_detail_internal` ADD COLUMN `pph21_adjustment` DECIMAL(15,2) NOT NULL DEFAULT 0.00 AFTER `pph_21`");
		}

		if(!$this->db->field_exists('pph21_adjustment_keterangan', 'payroll_slip_detail_internal')){
			$this->db->query("ALTER TABLE `payroll_slip_detail_internal` ADD COLUMN `pph21_adjustment_keterangan` VARCHAR(50) NULL AFTER `pph21_adjustment`");
		}
	}
	...
	private function getPayrollApprovalTotal($payroll_slip_id)
	{
		$row = $this->db->query("select coalesce(sum(total_pendapatan), 0) as total_nominal
			from payroll_slip_detail_internal
	...
	public function add_data($post)
	{
	    ...
	    if (!$header) {
	        ...
	        $this->db->update('payroll_slip_internal', [
	        	'status' => null,
	        	'status_id' => $isSubmitFinal ? 1 : 0,
	        	'rfu_reason' => '',
	        	'reject_reason' => '',
	...
	private function getPayrollComponentMap($bulan, $tahun)
	{
		...
		if ($bonusHeader) {
			...
			$rows = $this->db->where('bonus_internal_id', $bonusHeader->id)->get('bonus_internal_detail')->result();
			foreach ($rows as $row) {
		...
		if ($thrHeader) {
			...
			$rows = $this->db->where('thr_internal_id', $thrHeader->id)->get('thr_internal_detail')->result();
			foreach ($rows as $row) {
