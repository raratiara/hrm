...
class Tidakabsenmasuk_model extends MY_Model
{
	/* Module */
 	protected $folder_name				= "check_absensi/tidakabsenmasuk";
	...
	public function get_list_data()
	{
		...
		foreach($rResult as $row)
		{
			...
			$time_in 	= "";
			$time_out 	= "";
			if($row->shift_type == 'Reguler'){
				$dt = $this->db->query("select * from master_shift_time where name = 'Reguler'")->result(); 

				$attendance_type = 'Reguler';
				$time_in 	= $dt[0]->time_in;
				$time_out 	= $dt[0]->time_out;

			}else if($row->shift_type == 'Shift'){
