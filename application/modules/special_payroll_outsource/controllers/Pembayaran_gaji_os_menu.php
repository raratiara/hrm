...
class Pembayaran_gaji_os_menu extends MY_Controller
{
	/* Module */
 	const  LABELMODULE				= "pembayaran_gaji_os_menu"; // identify menu
 	const  LABELMASTER				= "Menu Pembayaran Gaji Outsource";
 	const  LABELFOLDER				= "special_payroll_outsource"; // module folder
 	const  LABELPATH				= "pembayaran_gaji_os_menu"; // controller file (lowercase)
 	const  LABELNAVSEG1				= "special_payroll_outsource"; // adjusted 1st sub parent segment
	...
	//============================== Additional Method ==============================//
 	...
 	public function getDaftarGajiOS(){
		...
		function generateSheet($sheetName, $dataset, $periode, $project){
		    ...
		    echo '<Row>
		            <Cell ss:MergeAcross="4" ss:StyleID="Title">
		                <Data ss:Type="String">DATA GAJI PT.MANDIRI AGANGTA SEJAHTERA</Data>
		            </Cell>
		          </Row>';
		    ...
		    foreach ($dataset as $row){
		        ...
		        echo '<Cell><Data ss:Type="Number">'.$row->gaji_bersih.'</Data></Cell>';
		        echo '</Row>';
		    }
		...
		header("Content-Disposition: attachment; filename=Daftar Gaji - ".$safeProjectName.".xls");
 	...
 	public function getDaftarGajiOS_old(){

	    $sql = "
	        select b.full_name, a.gaji_bersih, b.bank_acc_no, b.bank_name
			from special_payroll_slip_detail a
			left join employees b on b.id = a.employee_id
			where a.payroll_slip_id = ".$_GET['payroll_id']."
			order by b.full_name asc
	    ...
	    $getperiode = $this->db->query("select a.*, concat(b.name_indo,' ',a.tahun_penggajian) as periode_penggajian				, c.project_name  
						from special_payroll_slip a left join master_month b on b.id = a.bulan_penggajian
						left join project_outsource c on c.id = a.project_id
						where a.id = ".$_GET['payroll_id']." ")->result();

	    $periode_penggajian = $getperiode[0]->periode_penggajian;
	    $project_name 		= $getperiode[0]->project_name;
	    ...
	    echo '<Worksheet ss:Name="Daftar Gaji Outsource">
	            <Table>';
	    ...
	    echo '<Row>
	            <Cell ss:MergeAcross="4" ss:StyleID="Title2">
	                <Data ss:Type="String">Periode : '.$periode_penggajian.'</Data>
	            </Cell>
	          </Row>';
	    echo '<Row>
	        <Cell ss:MergeAcross="4" ss:StyleID="Title2">
	            <Data ss:Type="String">Project : '.$project_name.'</Data>
	        </Cell>
	   	</Row>';
