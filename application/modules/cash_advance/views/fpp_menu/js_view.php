        ...
        success: function(data)
        {
			if(data != false){
				if(save_method == 'update'){ 
					...
					$('[name="fpp_number"]').val(data.rowdata.ca_number);
					...
					if(data.rowdata.document != '' && data.rowdata.document != null){
						...
						document.getElementById("file-doc-fpp").innerHTML = '';

						const fileName = data.rowdata.document; // ini bisa dari PHP atau hasil upload
					    const fileUrl = baseUrl+"/uploads/cashadvance/fpp/" + fileName;
					    ...
					    document.getElementById("file-doc-fpp").appendChild(link);
					}else{
						$('span.file_doc').html('');
					}
				...
				if(save_method == 'detail'){ 
					$('span.fpp_number').html(data.rowdata.ca_number);
					...
					$('span.fpp_type').html(data.rowdata.fpp_type);
					...
					if(data.rowdata.document != '' && data.rowdata.document != null){
						// CLEAR link sebelumnya
						document.getElementById("file-link-fpp").innerHTML = '';
					    ...
					    document.getElementById("file-link-fpp").appendChild(link);
					}else{
						$('span.file').html('-');
					}
