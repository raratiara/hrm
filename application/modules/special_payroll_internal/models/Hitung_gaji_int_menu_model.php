...
class Hitung_gaji_int_menu_model extends MY_Model
{
	...
	{
		$this->db->query("ALTER TABLE `special_payroll_slip_detail_internal` ADD COLUMN IF NOT EXISTS `pph21_adjustment` DECIMAL(15,2) NOT NULL DEFAULT 0.00 AFTER `pph_21`, ADD COLUMN IF NOT EXISTS `pph21_adjustment_keterangan` VARCHAR(50) NULL AFTER `pph21_adjustment`");
	}
	...
	private function getPph21AdjustmentMap($bulan, $tahun, $employeeIds = [])
	{
		...
		$rows = $this->db
			...
			->get('special_spt_pph21_adjustment')
			->result();
	...
	public function edit_data($post)
	{
	    ...
	    if (isset($post['hdnempid_gaji'])) {
	    	$headerPayroll = $this->db->where('id', $post['id'])->get('special_payroll_slip_internal')->row();
