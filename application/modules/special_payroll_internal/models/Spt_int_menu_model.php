...
class Spt_int_menu_model extends MY_Model
{
	...
	{
		$this->db->query("
			CREATE TABLE IF NOT EXISTS `special_spt_pph21_adjustment` (
		...
		");
		$this->db->query("ALTER TABLE `special_spt_pph21_adjustment` ADD COLUMN IF NOT EXISTS `spt_pph21_id` INT(11) NULL AFTER `id`, ADD COLUMN IF NOT EXISTS `spt_pph21_detail_id` INT(11) NULL AFTER `spt_pph21_id`, ADD COLUMN IF NOT EXISTS `kurang_lebih_bayar` DECIMAL(15,2) NOT NULL DEFAULT 0.00 AFTER `amount`, ADD COLUMN IF NOT EXISTS `created_at` DATETIME DEFAULT NULL, ADD COLUMN IF NOT EXISTS `created_by` INT(11) DEFAULT NULL, ADD COLUMN IF NOT EXISTS `updated_at` DATETIME DEFAULT NULL, ADD COLUMN IF NOT EXISTS `updated_by` INT(11) DEFAULT NULL");
		$this->db->query("ALTER TABLE `special_spt_pph21_adjustment` CHANGE COLUMN IF EXISTS `tax_year` `tahun_pajak` VARCHAR(10) NULL");
		$this->db->query("ALTER TABLE `special_spt_pph21_adjustment` MODIFY COLUMN `amount` DECIMAL(15,2) NOT NULL DEFAULT 0.00");
		$this->db->query("ALTER TABLE `special_spt_pph21_adjustment` MODIFY COLUMN `proses_ke_bulan_penggajian` INT(2) NOT NULL DEFAULT 1");
		$this->db->query("CREATE UNIQUE INDEX IF NOT EXISTS `uniq_spt_adjustment_detail` ON `special_spt_pph21_adjustment` (`spt_pph21_detail_id`)");
	}

	private function syncPph21Adjustments($spt_id)
	{
		$header = $this->db->where('id', $spt_id)->get('special_spt_pph21_internal')->row();
		if(!$header) return;

		$rows = $this->db->query("
			select * from special_spt_pph21_detail_internal
			where spt_pph21_id = ?
		...
		foreach($rows as $row){
			...
			$existing = $this->db->where(['spt_pph21_detail_id' => $row->id])->get('special_spt_pph21_adjustment')->row();
			if($existing){
				$this->db->where(['spt_pph21_detail_id' => $row->id])->update('special_spt_pph21_adjustment', [
					'employee_id' => $row->employee_id,
					'tahun_pajak' => $header->tahun,
					'type' => $type,
					'amount' => $amount,
				...
				$this->db->insert('special_spt_pph21_adjustment', $item);
			}
		}
		...
		$this->db->delete('special_spt_pph21_adjustment');
	}
