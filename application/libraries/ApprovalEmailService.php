...
class ApprovalEmailService {
    ...
    public function sendtoRequester($menu, $trx_id, $requester1, $requester2)
    { 
        if($menu == 'cash_advance'){ 
            $data = $this->CI->db->query("
                        select *, ca_number as doc_num
                        FROM cash_advance 
                        WHERE id = ?
                    ", [$trx_id])->row();
        ...
        if ($requester1 != '' && $requester2 != '') { 
            $approver = $this->CI->db->query("select GROUP_CONCAT(distinct(personal_email)) AS emails, GROUP_CONCAT(distinct(full_name) SEPARATOR ' / ') AS names from employees where id in ('".$requester1."','".$requester2."') ")->result();

        } else if($requester1 != '' && $requester2 == '') { 
            $approver = $this->CI->db->query("select personal_email AS emails, full_name AS names from employees where id = '".$requester1."' ")->result();
            
        } else if($requester2 != '' && $requester1 == '') { 
            $approver = $this->CI->db->query("select personal_email AS emails, full_name AS names from employees where id = '".$requester2."' ")->result();
            
        }



        if (empty($approver) || empty($approver[0]->emails)) {
            return false;
        }
